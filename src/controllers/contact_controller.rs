...
async fn index(db: web::Data<Database>) -> impl Responder {
    let res = DbCollection::<Contact>::new(db.as_ref()).index().await;
    web::Json(res)
}

#[get("/count/")]
async fn count(db: web::Data<Database>) -> impl Responder {
    let res = DbCollection::<Contact>::new(db.as_ref()).count(None).await;
    match res {
...
async fn get(id: web::Path<String>, db: web::Data<Database>) -> impl Responder {
    ...
    match id_param {
        Ok(id_val) => match DbCollection::<Contact>::new(db.as_ref())
            .find_by_id(id_val)
            .await
        {
...
#[post("/")]
async fn create(value: web::Json<Contact>, db: web::Data<Database>) -> impl Responder {
    match DbCollection::<Contact>::new(db.as_ref())
...
pub fn cfg_contact_controller(cfg: &mut web::ServiceConfig) {
    cfg.service(index);
    cfg.service(count);
    cfg.service(get);
    cfg.service(create);
